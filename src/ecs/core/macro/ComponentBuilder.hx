...
class StorageInfo {
		...
		return switch (storageType) {
			...
			case TAG:
				...
				sure ? macro $containerFullNameExpr.worlds[$entityExpr.worldId].storage : macro @:privateAccess ecs.Workflow.world($entityExpr.worldId)
					.getTag($entityExpr, $te) ? $containerFullNameExpr.worlds[$entityExpr.worldId].storage : null;
		};
	}
	...
	public function getIsExpr(entityVar):Expr {
		...
		var existsExpr = [];
		if (t_type != null) {
			for (k in ComponentBuilder.componentTypeNames()) {
				var info = ComponentBuilder.containerInfo(k);
				var c_type = info.followedCT.toTypeOrNull(Context.currentPos());
				if (c_type.unifiesWith(t_type))
					existsExpr.push(info.getExistsExpr(entityVar));
			}
		}
