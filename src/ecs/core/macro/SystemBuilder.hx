...
class SystemBuilder {
	...
	public static function build(debug:Bool = false) {
			...
			switch (field.kind) {
				case FFun(func):
					switch (field.name) {
						...
						case '__initialize__':
							Context.error('Do not override the `__initialize__` function!', field.pos);
				...
				case FVar(t, e):
					switch (field.name) {
						case '__world__':
							Context.error('Do not override the `__world__` variable!', field.pos);
						case '__world_id__':
							Context.error('Do not override the `__world_id__` variable!', field.pos);
						default:
					}
		...
		fields.push(fvar([], [], '__world__', macro :ecs.World, null, Context.currentPos()));
								...
								if (path.name == "View") {
									...
									if (vr != null) {
										if (definedViews.find(function(v) return v.view.spec.name == vr.spec.name) == null) {
											...
											field.kind = FVar(vr.ct, null);
											initExpr.push(macro $i{field.name} = $x.inst(__world_id__));
						...
						if (components != null && components.length > 0) {
							...
							if (vs != null) {
								...
								if (view == null || view.varname != lcName) {
									if (view != null)
										trace('? vs ${view.varname} vs ${lcName}');
									var vr = ViewBuilder.getViewRec(vs, field.pos);
									...
									if (vr != null) {
										...
										fields.push(fvar([], [], lcName, vr.ct, null, Context.currentPos()));
										initExpr.push(macro $i{lcName} = $tp.inst(__world_id__));
									} else {
		...
		var uexprs = []
		#if ecs_profiling.concat
		([macro var __timestamp__ = haxe.Timer.stamp()])
		#end
			...
			return switch (f.type) {
				...
				case VIEW_ITER: {
						...
						var cache = f.view.spec.includes.map(function(c) {
							var info = getComponentContainerInfo(c.ct, pos);
							return info.getCacheExpr(macro __world_id__, info.fullName + "_inst");
						}).filter((x) -> x != null);
...
//					trace('Activating: ${v.varname}');
		...
		var dexpr = macro if (activated) $b{
			...
			.concat( // deactivate views
				definedViews.map(function(v) {
					return macro $i{v.varname}.deactivate(__world_id__);
				}))
